...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        'group/card gap-[var(--card-spacing)] py-[var(--card-spacing)] text-card-foreground has-data-[slot=card-footer]:pb-0 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-xl *:[img:last-child]:rounded-b-xl flex flex-col overflow-hidden rounded-2xl bg-white text-sm shadow-sm [--card-spacing:1rem] has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:0.75rem]',
        className,
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'bg-muted/50 p-[var(--card-spacing)] flex items-center rounded-b-xl border-t',
        className,
      )}
      {...props}
    />
