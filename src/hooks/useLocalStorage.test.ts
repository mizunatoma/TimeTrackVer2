...
describe('useLocalStorage', () => {
  ...
  it('setで保存すると、値とlocalStorageの両方が更新される', () => {
    const { result } = renderHook(() =>
      useLocalStorage('todoPanelToggle', false),
    )
    ...
    expect(localStorage.getItem('todoPanelToggle')).toBe(JSON.stringify(true)) // localStorageの値も確認
  })

  it('既に保存された値があれば、マウント時にその値で復元される', () => {
    // 1. 前回保存された状態を復元（先にlocalStorageへ書き込む）
    localStorage.setItem('todoPanelToggle', JSON.stringify(true))
